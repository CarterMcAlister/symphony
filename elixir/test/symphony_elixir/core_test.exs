defmodule SymphonyElixir.CoreTest do
  ...
  test "prompt builder can render an alternate research workflow file" do
    research_workflow_path = Workflow.research_workflow_file_path()
    File.write!(research_workflow_path, "Research packet for {{ issue.identifier }} attempt={{ attempt }}\n")

    issue = %Issue{
      identifier: "MT-781",
      title: "Research workflow prompt",
      description: "Use the sibling workflow file",
      state: "Todo",
  ...
  test "agent runner stops after a Human Review wait turn instead of consuming continuation turns" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-agent-runner-human-review-wait-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      state_fetcher = fn [_issue_id] ->
        attempt = Process.get(:human_review_wait_fetch_count, 0) + 1
        Process.put(:human_review_wait_fetch_count, attempt)
        send(parent, {:issue_state_fetch, attempt})

        {:ok,
         [
           %Issue{
             id: "issue-human-review-wait",
             identifier: "MT-514",
             title: "Wait in Human Review",
             description: "Should not recurse through continuation turns",
             state: "Human Review"
           }
         ]}
      end
  ...
  test "agent runner runs the research workflow before the implementation workflow when present" do
    test_root =
      Path.join(
        System.tmp_dir!(),
    ...
    try do
      ...
      state_fetcher = fn [_issue_id] ->
        attempt = Process.get(:research_phase_fetch_count, 0) + 1
        Process.put(:research_phase_fetch_count, attempt)
        send(parent, {:issue_state_fetch, attempt})
        ...
        {:ok,
         [
           %Issue{
             id: "issue-research",
             identifier: "MT-782",
             title: "Run research before implementation",
      ...
      assert length(turn_texts) == 2
      assert Enum.at(turn_texts, 0) == "Research packet for MT-782"
